...
class PredictionService {
  ...
  static Future<PredictionResult> predictDisease({
    ...
    try {
      ...
      // Check for valid URL
      if (ApiConstants.predictUrl.isEmpty) {
        return PredictionResult.error('Prediction API URL is not set.');
      }
      ...
      if (response.statusCode == 200) {
        if (response.body.isEmpty) {
          return PredictionResult.error('Empty response from server.');
        }
        Map<String, dynamic> jsonResponse;
        try {
          jsonResponse = json.decode(response.body);
        } catch (_) {
          return PredictionResult.error('Invalid response from server.');
        }
        ...
        // Return result with image path (if copyWith exists)
        try {
        ...
        String errorMsg = 'Invalid image format. Please upload a clear leaf photo.';
        try {
          final errorBody = json.decode(response.body);
          errorMsg = errorBody['message'] ?? errorMsg;
        } catch (_) {}
        return PredictionResult.error(errorMsg);
